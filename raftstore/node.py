...
class RaftNode:
    ...
    def _replicate_peer(self, peer):
        ...
        with self.lock:
            if response["term"] > self.term:
                self._become_follower(response["term"]); return
            if self.role != "leader" or self.term != sent_term: return
    ...
    def replicate_all(self):
        threads = [threading.Thread(target=self._replicate_peer, args=(p,), daemon=True) for p in self.peers]
    ...
    def put(self, key, value, timeout=2.5):
        with self.lock:
            ...
            index = len(self.log)
            self.log.append({"term": self.term, "op": "set", "key": key, "value": value})
            self._persist()
    ...
    def status(self):
        with self.lock:
            return {"node_id": self.node_id, "role": self.role, "term": self.term,
